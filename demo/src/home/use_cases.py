...
def log_with_timing_2():
    try:
        with begin_span(state={"foo": "bar"}, on_begin=LogBegin("debug")):
            sleep(random.uniform(0.5, 1.0))
...
def log_single_loop_3():
    with begin_span(on_end=LogEnd("info")):
        test_stats = LoopStats()
        for i in [1, 2, 3]:
            with begin_span(index=i, on_end=CountEvent(test_stats)):
                sleep(random.uniform(0.5, 1.5))
                log_debug("This item is complete.")
        log_info("Test stats.", test_stats=test_stats)
